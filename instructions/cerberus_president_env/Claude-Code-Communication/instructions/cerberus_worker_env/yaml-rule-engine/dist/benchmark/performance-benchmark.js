...
class PerformanceBenchmark {
    ...
    // シンプルな評価のベンチマーク
    async benchmarkSimpleEvaluation() {
    ...
    // 複雑な評価のベンチマーク
    async benchmarkComplexEvaluation() {
    ...
    // 並列評価のベンチマーク
    async benchmarkParallelEvaluation() {
    ...
    // ベンチマーク実行ヘルパー
    async runBenchmark(testName, iterations, fn) {
        const times = [];
    ...
    printResults() {
        ...
        for (const result of this.results) {
            console.log(result.testName.padEnd(30) +
    ...
    analyzePerformance() {
        ...
        const memoryEfficiency = this.results.map(r => r.totalEvaluations / r.memoryUsage.heapUsed).reduce((a, b) => a + b, 0) / this.results.length;
