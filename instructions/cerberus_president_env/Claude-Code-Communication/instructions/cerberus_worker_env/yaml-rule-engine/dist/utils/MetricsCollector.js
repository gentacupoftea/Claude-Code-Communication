...
class MetricsCollector extends events_1.EventEmitter {
    ...
    recordStageMetric(stageName, success, latency) {
        if (!this.config.enabled)
            return;
    ...
    async export() {
        ...
        logger_1.logger.info('Exporting metrics', {
        ...
        for (const [stage, metric] of metrics.stageMetrics) {
            logger_1.logger.info('Stage metrics', {
                stage,
    ...
    reset() {
        ...
        logger_1.logger.info('Metrics reset');
    }
    shutdown() {
        if (this.exportTimer) {
    ...
    // ヒストグラム用のバケット生成
    getLatencyHistogram(buckets = [10, 25, 50, 100, 250, 500, 1000, 2500, 5000]) {
