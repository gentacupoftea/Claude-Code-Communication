...
import { Calendar, Filter, Download, TrendingUp, AlertCircle } from 'lucide-react';
...
const AnalyticsDashboard: React.FC = () => {
  ...
  const isValidDateRange = useMemo(() => {
    return isValid(dateRange.startDate) && 
  ...
  const { data: orderSummary, isLoading: orderLoading, error: orderError } = useQuery({
  ...
  const handleExport = useCallback(async (dataType: string) => {
    ...
    try {
      ...
      const filename = filenameMatch ? filenameMatch[1] : `${dataType}_${Date.now()}.${exportFormat}`;
  ...
  // Loading state
  const isLoading = orderLoading || categoryLoading || trendLoading || geoLoading;
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
            ...
            {orderLoading ? (
              ...
              <div className="h-64 flex items-center justify-center">
                <p className="text-red-500">{t('analytics.error.chart_load_failed')}</p>
              </div>
            ) : (
