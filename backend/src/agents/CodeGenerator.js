...
class CodeGenerator {
  ...
  async planFeatureDevelopment(intent) {
    ...
    const phases = [
      {
        name: 'データモデル設計',
        files: [`src/models/${featureName}Model.js`],
    ...
    if (components.includes('API') || components.includes('backend')) {
      phases.push({
        name: 'API実装',
        files: [
          `src/routes/${featureName}Routes.js`,
          `src/controllers/${featureName}Controller.js`,
    ...
    if (components.includes('UI') || components.includes('frontend')) {
      phases.push({
        name: 'フロントエンド実装',
        files: [
          `src/components/${featureName}Component.jsx`,
    ...
    phases.push({
      ...
      files: [
        ...
        `tests/integration/${featureName}.test.js`
      ],
